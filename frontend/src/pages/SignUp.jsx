...
dispatch(setUserData(result.data))
    ...
    <div className='w-full h-screen bg-slate-200 flex items-center justify-center'>
      <div className='w-full max-w-125 h-150 bg-white rounded-lg shadow-gray-400 shadow-lg flex flex-col gap-7.5'>
        ...
        <form className='w-full flex flex-col gap-5 items-center' onSubmit={handleSignUp}>
          <input type="text" placeholder="username" className='w-[90%] h-12.5 outline-none border-2 border-[#20c7ff] px-5 py-2.5 bg-[white] rounded-lg shadow-gray-200 shadow-lg text-gray-700 text-[19px] ' onChange={(e)=>setUserName(e.target.value)} value={userName}/>
  ...
  <p className="text-red-500 font-semibold text-[18px]">
    * {err}
          ...
          <button className='px-5 py-2.5 bg-[#20c7ff] rounded-2xl shadow-gray-400 shadow-lg text-[20px] w-50 mt-5 font-semibold hover:shadow-inner' disabled={loading}>{loading?"Loading...":"Sign Up"}</button>
